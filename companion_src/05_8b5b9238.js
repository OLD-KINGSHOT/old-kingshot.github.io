...
const SITUATIONS=[
 {key:'bear-trap',name:'Bear Trap',sub:'Bear Hunt \u00b7 harian',icon:'\u25c8',kind:'rally',role:'Joiner',evType:'bear',minDay:0,daily:true,pick:1,
  ...
  do:'Bawa 1 hero ber-Lethality di slot-1: Chenko (Cav) / Yeonwoo (Archer) / Amane (Attack). Cuma skill Expedition #1 hero PERTAMA yg dihitung saat join (hero 2-3 cuma nambah kapasitas troop). ⛔ JANGAN kirim hero defensif/ekonomi sbg JOINER slot-1 (Gordon/Diana/Long Fei/Helga/Jabel) — cuma skill #1 yg dihitung, troop-only lebih baik. TAPI Jabel/Quinn/Howard OK sbg FILLER saat kamu HOST (butuh 3 hero isi 3 kelas; Quinn bawa skill damage). Troop: ARCHER terkuat (beruang infantry \u2192 +10%). Forgehammer #1. \u26a0 Aliansi pasang 2 TRAP (jam beda) tapi kamu hanya bisa serang SATU KALI \u2014 pilih BT 1 ATAU BT 2 yang pas jam onlinemu; sudah ikut satu = BT berikutnya baru muncul lagi siklus depan. 🚩 Kamu = JOINER (trap di-HOST R4/R5 — itu sebabnya bukan rally-lead). Kalau KAMU yang HOST: ke-3 skill hero-mu berlaku ke SELURUH rally (beda dari joiner) → bawa 3 hero OFENSIF terkuat (Gen 1: Amadeus / infantry terkuat; hindari Diana) + pet Mighty Bison (+15k kapasitas). R4/R5: STOP donasi arrow saat Pitfall Lv5 (lebihnya terbuang). ⚔ FULL atau SPLIT? Tiap march ISI PENUH (max kapasitas + pet Mighty Bison) — JANGAN pecah 1 march jadi march kecil (under-fill = buang damage). Punya >1 march queue (VIP6+)? Kirim tiap march PENUH ke rally BERBEDA sekaligus (2-3 rally) — leverage tertinggi. Selama 30 mnt: re-join SEMUA rally yang bisa (kemenangan = sering & banyak).'},
